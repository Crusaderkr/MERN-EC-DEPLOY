...
app.post("/upload", upload.single('product'), (req, res) => {
  ...
  res.json({
    success: 1,
    image_url: `/images/${req.file.filename}` // Return the correct path
  });
});
...
const fetchUser = async (req, res, next) => {
  ...
  if (!token) {
    res.status(401).send({ errors: "Please authenticate using your registered ID" });
  } else {
    try {
      ...
      res.status(401).send({ errors: "Please authenticate with valid ID" });
    }
  }
}
